...
* [2021-06-26 Sat 21:05] 配置 default
  ...
  #+begin_src emacs-lisp :noweb-ref pref
    ...
    (defmacro mel/expand-pkgs-file (file-name)
...
* [2021-06-27 Sun 22:32] 设置 org mode
  ...
  #+begin_src emacs-lisp :noweb-ref org-config
    ...
    (setq org-todo-keywords (quote ((sequence
                                     ...
                                     "TODO(t@)"
                                     ;; working on it. logging the time of entring and leaving
                                     "DOING(o!/!)"
...
** [2022-05-20 Fri 14:30] org config 中添加 back ref 功能
   ...
   - [2023-09-06 Wed 15:49] back ref 的 heading 更应该叫 FYI -- for your interest

   #+begin_src emacs-lisp :noweb-ref org-config-obsolete
     ...
     info if possible."
       ...
       (let* ((from-link
         ...
         (save-restriction
           ...
           ;; goto `ref/FYI' headline
           (while (and (< (point) (point-max)) (not found-p))
             ;; we assume the depth of this subtree is not greater than
             ;; 2(include itself).
             (org-next-visible-heading 1)
             (setq to-heading (nth 4 (org-heading-components)))
             (if (or (string= ref-heading (string-trim to-heading))
                     (string= fyi-heading (string-trim to-heading)))
                 (setq found-p t)))
           ;; ref/FYI entry cannot been found
           (when (not found-p)
             ...
             ;; unless the target entry has sub-heading, we demote ref/FYI as
             ;; subtree of current heading
             (while (> (nth 0 (org-heading-components)) to-level)
               (org-promote-subtree))
             (org-demote-subtree)
             ;; insert ref/FYI headling
             (org-edit-headline fyi-heading)
             ;; insert link
             (insert "\n" "- " from-link-str))
           ;; insert link in the ref/FYI entry
           (when found-p
             ;; goto beginning of item
             (condition-case err
                 (org-list-search-forward (org-item-beginning-re))
               (error
                (message "no list item in `ref/FYI' heading, item will be inserted at the end of the entry")
                ;; no list item in ref/FYI, insert one.
                ;; we are at the beginning of the ref/FYI heading
                (end-of-line)
                (insert "\n" "- " from-link-str)
   ...
   #+end_src
   :REF:
   - [[id:node-2023-11-07-19-39-00-000000][更改 back-ref 功能]]
   - [[id:node-2023-11-21-14-02-00-000000][将 back-ref 中不同信息放到对应的 drawer 中]]
   :END:
...
** <2023-11-14 Tue 10:09> 给 insert drawer 添加一个函数
   :PROPERTIES:
   :ID:       node-2023-11-14-10-09-00-000000
   :ts:       <2023-11-14 Tue 10:09>
   :END:
   参照 org-insert-drawer

   :FYI:
   #+begin_src emacs-lisp :noweb-ref org-config

     (defun mel/org-goto-drawer (&optional goto)
       "Goto REF drawer.

     If there is no REF drawer in the section, append one at the end
     of the section.

     With a `\\[universal-argument]' prefix argument, goto FYI
     drawer."
       (interactive "P")
       (let* ((drw (if (equal goto '(4)) "FYI" "REF"))
              (regexp (concat "^[ \t]*:" drw ":[ \t]*$"))
              (drawer-end-re "^[ \t]*:END:[ \t]*$")
              pos-min pos-max)
         (save-restriction
           ...
           (setq pos-max
                 (min
                  (point-max)
                  (save-excursion (outline-next-visible-heading 1) (point))))
           (if (re-search-forward regexp pos-max 'move)
               ;; goto drawer
               (progn
                 ;; indent drawer
                 (org-indent-drawer)
                 ;; move to end of drawer
                 (re-search-forward drawer-end-re pos-max 'move)
                 ;; move to end of previous line
                 (end-of-line 0))
             ;; Insert new drawer
             (end-of-line 0)
             ;; inserting end of line unless it is a blank line
             (unless
                 (save-excursion
                   (beginning-of-line) (looking-at-p "[[:blank:]]*$"))
               (insert-before-markers "\n"))
             (setq pos-min (point))
             (insert-before-markers (concat ":" drw ":\n"))
             (insert-before-markers ":END:")
             (end-of-line 0)
             (org-indent-drawer)))))

     (substitute-key-definition 'org-insert-drawer 'mel/org-goto-drawer org-mode-map)
   #+end_src
   :END:

** <2023-11-07 Tue 19:39> 更改 back-ref 功能
   :PROPERTIES:
   :ID:       node-2023-11-07-19-39-00-000000
   :ts:       <2023-11-07 Tue 19:39>
   :MAP_POS:  1
   :END:
   将 back-ref 放到 FYI drawer 中。需要调整一下 `mel/org-back-ref-node'
   函数。
   :FYI:
   #+begin_src emacs-lisp :noweb-ref org-config-obsolete
     ...
     info if possible."
       ...
       (let* ((from-link
         ...
         (save-restriction
           (org-narrow-to-subtree)
           (setq to-limit (min (point-max) (save-excursion (org-next-visible-heading 1) (point))))
           ;; goto `ref/FYI' drawer
           (let ((regexp (concat "^[ \t]*:" fyi-drawer ":[ \t]*$"))
                 (end to-limit)
                 (case-fold-search t))
             (unless (not (re-search-forward regexp end 'move))
               (setq to-limit (save-excursion (re-search-forward drawer-end-re end t)))
               (setq found-p t)))
           ;; ref/FYI entry cannot been found, add FYI drawer
           (when (not found-p)
             ;; move to end of previous line
             (if (org-at-heading-p) (end-of-line 0))
             (unless
                 (save-excursion
                   (beginning-of-line) (looking-at-p "[[:blank:]]*$"))
               (insert-before-markers "\n"))
             (insert-before-markers (concat ":" fyi-drawer ":\n"))
             (setq to-pos (save-excursion (end-of-line 0) (point)))
             (insert-before-markers ":END:")
             ;; reuse `to-limit' for drawer
             (setq to-limit (point))
             (end-of-line 0)
             (setq found-p t))
           ;; insert link in the ref/FYI entry
           (when found-p
             ;; goto beginning of item
             (condition-case err
                 (org-list-search-forward (org-item-beginning-re) to-limit)
               (error
                (message "no list item in `ref/FYI' drawer, item will be inserted at the end of the entry")
                ;; no list item in ref/FYI, insert one.
                (insert "\n" "- " from-link-str)
                (goto-char to-pos)
                (org-indent-drawer)
             ...
             (when found-p
               (condition-case err
                   (org-list-search-forward (plist-get from-link :link) to-limit)
                 ;; there is no duplicated link. Insert the link to the list
                 (error
                  (org-insert-item)
                  (insert from-link-str)
   ...
   :END:
   :REF:
   - [[id:node-2023-11-02-10-24-00-000000][[2023-11-02 Thu 10:24] 考虑将 FYI/ref 放到 drawer 中]]
   :END:

** <2023-11-21 Tue 14:02> 将 back-ref 中不同信息放到对应的 drawer 中
   :PROPERTIES:
   :ID:       node-2023-11-21-14-02-00-000000
   :ts:       <2023-11-21 Tue 14:02>
   :MAP_POS:  3
   :END:
   每个 heading 下可以有多个 drawer，可以在 FYI 里放啰嗦的信息，在 REF 中
   放链接。
   :FYI:
   #+begin_src emacs-lisp :noweb-ref org-config
     ...
     info if possible."
       ...
       (let* ((from-link
         ...
         (save-restriction
           ...
           (setq to-limit (min (point-max) (save-excursion (org-next-visible-heading 1) (point))))
           ;; goto `ref/FYI' drawer
           (let ((regexp (concat "^[ \t]*:" ref-drawer ":[ \t]*$"))
                 (end to-limit)
                 (case-fold-search t))
             (when (re-search-forward regexp end 'move)
               (setq to-limit (save-excursion (re-search-forward drawer-end-re end t)))
               (setq found-p t)))
           ;; ref/FYI entry cannot been found, add FYI drawer
           (unless found-p
             ...
             (unless
                 (save-excursion
                   (beginning-of-line) (looking-at-p "[[:blank:]]*$"))
               (insert-before-markers "\n"))
             (insert-before-markers (concat ":" ref-drawer ":\n"))
             (setq to-pos (save-excursion (end-of-line 0) (point)))
             (insert-before-markers ":END:")
             ;; reuse `to-limit' for drawer
             (setq to-limit (point))
...
* [2023-12-04 Mon 11:32] 更新状态名
  :PROPERTIES:
  :ID:       node-2023-12-04-11-32-00-000000
  :ts:       <2023-12-04 Mon 11:32>
  :END:
  状态名应该包含 UNREAD, TODO, DOING, WAIT, DONE, CANCELED.

  DOING 标识永远只有一个，表示进在进行的事项，所有未进行的待办事项都是
  WAIT 状态。
