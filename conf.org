    ...
    (org-with-point-at pom
      ...
      (unless (org-before-first-heading-p)
        (let* ((id (org-entry-get nil "ID"))
          ...
          ;; set id if not eq
          (unless (and id (string= id id-str))
            (org-entry-put pom "ID" id-str)
            (org-id-add-location id-str (buffer-file-name (buffer-base-buffer))))
...
#+begin_src emacs-lisp :noweb-ref org-config
  ...
  (defun mel/org-copy-id (&optional pom)
    "copy id of the entry at POM. if POM is undefined, copy id of
    the entry at pointer"
    (interactive)
    (org-with-point-at pom
      (let ((str (org-entry-get nil "ID" t)))
        (unless (and str (string-empty-p str))
          (mel/org-set-node-id)
          (setq str (org-entry-get nil "ID" t)))
        ;; if 'str' is undefined, use headline
        (if (or (not str) (string-empty-p str))
          (setq str (nth 4 (org-heading-components))))
        (org-kill-new str))))
  ;; bind keys
