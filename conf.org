...
* [2021-06-27 Sun 22:32] 设置 org mode
  ...
  - =inbox.org= 文件用来记录待办事项。
    ...
    #+begin_src emacs-lisp :noweb-ref org-config
      ;; capture inbox
      (setq mel/org-inbox-file (mel/expand-org-file "inbox.org"))
    #+end_src
  ...
  - =material.org= 文件用来记录平时遇到问题的解决方法，作用类似于一个工具书。
    ...
    #+begin_src emacs-lisp :noweb-ref org-config
      ;; capture material
      (setq mel/org-material-file (mel/expand-org-file "material.org"))
    #+end_src
  ...
  - =journal.org= 类似于日记。
    ...
    #+begin_src emacs-lisp :noweb-ref org-config
      ;; capture journal
      (setq mel/org-journal-file (mel/expand-org-file "journal.org"))
    #+end_src
  ...
  - =project.org= 更正式一点，因为是个长期工作，需要考虑它是什么，使用
    ...
    #+begin_src emacs-lisp :noweb-ref org-config
      ;; capture project
      (setq mel/org-prj-file (mel/expand-org-file "project.org"))
    #+end_src
  ...
  - 日常工作中每天还要写周报，加个 =review.org= ，记录每周周报。日常工作中
    ...
    #+begin_src emacs-lisp :noweb-ref org-config
      ...
      (setq mel/org-review-file (mel/expand-org-file "review.org"))
    #+end_src
...
** <2022-07-10 Sun 10:12> org mode 中添加 work-on 功能
   ...
   #+begin_src emacs-lisp :noweb-ref org-config
     ...
     (defun mel/org-work-on (;; inbox specific files
       ...
       (let ((mel/org-inbox
              (or inbox-file (mel/expand-org-file "inbox.org")))
             (mel/org-journal
              (or journal-file (mel/expand-org-file "journal.org")))
             (mel/org-review
              (or review-file (mel/expand-org-file "review.org")))
             (mel/org-prj
              (or prj-file (mel/expand-org-file "project.org")))
             (mel/org-note
              (or note-file mel/org-note-file))
             (mel/org-material
              (or material-file (mel/expand-org-file "material.org"))))
         ;; clear templates
         (setq org-capture-templates nil)
         ;; capture inbox
