...
** [2022-09-07 Wed 15:48] mel/org-work-on 这个功能是不是只需要两个目录就够了？
   ...
   #+begin_src emacs-lisp :noweb-ref org-config
     ...
     (defun mel/org-capture-workon (base-dir inbox-dir note-file)
       ...
       (interactive
        (let ((b-dir (read-directory-name "org base dir: " org-directory mel/org-file-dir))
              (i-dir (read-directory-name "captured inbox dir: " org-directory mel/org-file-dir))
              (n-file (read-file-name "note file: " org-directory org-default-notes-file)))
          (when (or
                 (not (file-directory-p b-dir))
                 (member b-dir (list "" nil)))
            (message "org base dir not changed: %s" mel/org-file-dir)
            (setq b-dir mel/org-file-dir))
          (when (or
                 (not (file-directory-p i-dir))
                 (member i-dir (list "" nil)))
            (message "captured inbox dir not changed: %s" mel/org-file-dir)
            (setq i-dir mel/org-file-dir))
          (when (or
                 (not (file-exists-p org-default-notes-file))
                 (member n-file (list "" nil)))
            (message "note file not changed: %s" org-default-notes-file)
