...
* [2021-06-26 Sat 21:05] 配置 default
  ...
  add a dir called 'garage' to store private elisp scripts. And put this
  dir on top of ~load-path~, so libraries in this dir take precedence over
  others.

  #+begin_src elisp :noweb-ref pref-garage
    ;; create dir `garage'
    (defvar mel/garage
      (mel/expand-emacs-d "garage")
      "Private elisp scripts directory")

    ;; create `garage' dir
    (mel/mkdir mel/garage)

    ;; add `garage' on top of load-path
    (let ((default-directory mel/garage))
      (setq load-path
            (append
